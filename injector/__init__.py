...
class InjectorContainer(knot.Container):
    ...
    def _scan_for_injections(self, item):
        _data = inspect.getargspec(item)
        _arguments = _data.args or []
        _defaults = _data.defaults or []
        _params = [
            k for k in _arguments if k not in dict(
                zip(_arguments[::-1], _defaults[::-1])
            )
        ]
